...
local function debugFrame_TabWords()
    if SnugUI.settings.debug then
        ...
        TabWordsFrame:SetBackdropColor(0, 0, 0, 1) -- change here for alpha
    end
end

SnugUI.buttons.tabWords = {}
local function RefreshTabWords()
    local shownTabs = {}
    for i = 1, NUM_CHAT_WINDOWS do
        local tab = _G["ChatFrame" .. i .. "Tab"]
        local frame = _G["ChatFrame" .. i]
        local text = GetChatWindowInfo(i)
        if tab and frame and tab:IsShown() and frame.isDocked then
            table.insert(shownTabs, { tab = tab, index = i, text = text})
        end
    end
    ...
    for _, info in ipairs(shownTabs) do
        ...
        local btn = SnugUI.buttons.tabWords[btnIndex]
        ...
        if not btn then
            ...
            btn.text = text
            SnugUI.buttons.tabWords[btnIndex] = btn
        end
        ...
        text:SetText(info.text)  -- Set actual tab label
    ...
    -- Hide unused buttons
    for i = btnIndex, #SnugUI.buttons.tabWords do
        SnugUI.buttons.tabWords[i]:Hide()
    end
end

local function initTabWords() -- script setup for tabwords
    for _, btn in ipairs(SnugUI.buttons.tabWords) do
        btn:SetSize(180, 20)
        ...
        btn:SetScript("OnEnter", function(self)
            for _, b in ipairs(SnugUI.buttons.tabWords) do
                if b:IsShown() then
        ...
        btn:SetScript("OnLeave", function(self)
            for _, b in ipairs(SnugUI.buttons.tabWords) do
                if b:IsShown() then
                    UIFrameFadeOut(b, 3, b:GetAlpha(), 0)
                end
            end
...
SnugUI.loginTrigger(function()
    ...
    --table.insert(SnugUI.commitRegistry, refreshtabwords2)
    --table.insert(SnugUI.commitRegistry, bruh)
end)
